...
namespace SADXFontEdit
{
	public partial class Form1 : Form
	{
		...
		LocalStrings ls = new LocalStrings();
		...
		{
			Encoding enc;
			if (russianToolStripMenuItem.Checked) enc = cp1251;
			else enc = cp1252;
			filename = filename_a;
			byte[] file = System.IO.File.ReadAllBytes(filename);
			int count = file.Length / 0x58;
			...
			{
				files.Add(new FontItem(file, i * 0x58));
				if (japaneseLayoutToolStripMenuItem.Checked) listBox1.Items.Add(files[i].ID.ToString("X4") + ' ' + alphabet[Math.Min(alphabet.Length - 1, i)]);
				else
				{
					byte[] origtext = new byte[] { (byte)i };
					string utf16String = enc.GetString(origtext);
					byte[] utf8Text = Encoding.UTF8.GetBytes(utf16String);
					string utf8String = Encoding.UTF8.GetString(utf8Text);
					listBox1.Items.Add(files[i].ID.ToString("X4") + ' ' + utf8String);
				}
			}
			listBox1.EndUpdate();
			extractAllToolStripMenuItem.Enabled = saveAsToolStripMenuItem.Enabled = true;
		...
		private void openToolStripMenuItem_Click(object sender, EventArgs e)
		{
			...
			{
				DefaultExt = "bin",
				Filter = "FONTDATA files|fontdata?.bin|All Files|*.*"
			};
		...
		private void extractAllToolStripMenuItem_Click(object sender, EventArgs e)
		{
			using (SaveFileDialog dlg = new SaveFileDialog() { DefaultExt = "", Filter = "", FileName = "font" })
			{
				if (dlg.ShowDialog(this) == DialogResult.OK)
				{
					...
					ini.sheets = new List<FontSheet>();
					for (int i = 0; i < (files.Count) / 256 + 1; i++)
					{
						FontSheet sheet = new FontSheet();
						sheet.id = new List<string>();
						sheet.md = new List<string>();
						...
						using (var canvas = Graphics.FromImage(bitmap))
						{
							...
							for (int u = 0; u < 256; u++)
							{
								...
								sheet.id.Add(files[256 * i + u].ID.ToString("X"));
								sheet.md.Add(BitConverter.ToString(files[256 * i + u].miscdata).Replace("-", ""));
								sheet.count++;
		...
		private void japaneseLayoutToolStripMenuItem_Click(object sender, EventArgs e)
		{
			japaneseLayoutToolStripMenuItem.Checked = true;
			latinToolStripMenuItem.Checked = false;
		...
		private void importToolStripMenuItem_Click(object sender, EventArgs e)
		{
			...
			})
			{
				if (a.ShowDialog(this) == DialogResult.OK)
				{
					...
					imageList1.Images.Clear();
					listBox1.BeginUpdate();
					for (int i = 0; i < ini.sheets.Count; i++)
					{
						FontSheet sheet = ini.sheets[i];
						Bitmap sheetbmp = new Bitmap(Image.FromFile(Path.GetDirectoryName(a.FileName) + "\\fontsheet" + i.ToString() + ".png"));
						for (int u = 0; u < sheet.count; u++)
						{
							FontItem item = new FontItem();
							Bitmap charbmp = new Bitmap(24, 24);
							using (var canvas = Graphics.FromImage(charbmp))
							{
								Text = "Importing sheet " + i.ToString() + " of " + ini.sheets.Count.ToString() + ", character " + u.ToString() + " of " + sheet.count.ToString();
								canvas.InterpolationMode = System.Drawing.Drawing2D.InterpolationMode.NearestNeighbor;
								...
								item.ID = ushort.Parse(sheet.id[u], System.Globalization.NumberStyles.HexNumber);
								item.miscdata = StrToByteArray(sheet.md[u]);
								files.Add(item);
							}
						}
					...
					{
						if (japaneseLayoutToolStripMenuItem.Checked) listBox1.Items.Add(files[z].ID.ToString("X4") + ' ' + alphabet[Math.Min(alphabet.Length - 1, z)]);
						else
						{
							if (russianToolStripMenuItem.Checked) enc = cp1251;
							else enc = cp1252;
							byte[] origtext = new byte[] { (byte)z };
							string utf16String = enc.GetString(origtext);
							byte[] utf8Text = Encoding.UTF8.GetBytes(utf16String);
							string utf8String = Encoding.UTF8.GetString(utf8Text);
							listBox1.Items.Add(files[z].ID.ToString("X4") + ' ' + utf8String);
						}
					}
					listBox1.EndUpdate();
		...
		{
			russianToolStripMenuItem.Checked = false;
			japaneseLayoutToolStripMenuItem.Checked = false;
			latinToolStripMenuItem.Checked = true;
		}
