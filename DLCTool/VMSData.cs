...
namespace DLCTool
{
    public class SA1DLC
    {
        public class VMSFile
        {
            ...
            /// </summary>
            public static VMSFile LoadFile(string filename)
            {
                switch (Path.GetExtension(filename.ToLowerInvariant()))
                {
                    case ".vms":
                        return new VMSFile(File.ReadAllBytes(filename));
                    case ".ini":
                        break;
                    default:
                ...
                VMSFile header = IniSerializer.Deserialize<VMSFile>(filename);
            ...
            public void Export(string dir, bool writeall = false)
            {
                ...
                IniSerializer.Serialize(new VMSFile(file), Path.Combine(dir, fname + ".ini"));
            ...
            /// Make a clone
            /// </summary>
            public VMSFile(VMSFile original)
            {
        ...
        public class VMIFile
		{
            ...
            {
                if (ResName.Length > 8)
                    System.Windows.Forms.MessageBox.Show("For the VMI file to work correctly, the VMS filename should be 8 characters or less.", "DLC Tool Warning", System.Windows.Forms.MessageBoxButtons.OK, System.Windows.Forms.MessageBoxIcon.Warning);
                ...
                FileName = "SONICADV_" + data.Identifier.ToString("D3");
                Size = (uint)data.GetBytes().Length;
            }
