...
namespace SAModel.GC
{
	...
	public class GCMesh
	{
		...
		/// <param name="labels">The labels of the primitives</param>
		/// <param name="indexFlags">The index attribute flags of the primitives</param>
		public GCMesh(byte[] file, int address, uint imageBase, Dictionary<int, string> labels, GCIndexAttributeFlags indexFlags)
		{
			...
			if (parametersCount != 0)
			{
				...
				{
					ParameterName = $"parameter_{parametersOffset:X8}";
				}
			}
			...
			if (primitivesSize != 0)
			{
				...
				{
					PrimitiveName = $"primitive_{primitivesOffset:X8}";
				}
			}
