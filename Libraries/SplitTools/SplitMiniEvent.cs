...
namespace SplitTools.SAArc
{
	public static class SA2MiniEvent
	{
		...
		public static void Split(string filename, string outputPath, string labelFile = null)
		{
			...
			try
			{
				...
				var ini = new MiniEventIniData { Name = Path.GetFileNameWithoutExtension(evfilename) };
				...
				{
					address = 8 + (4 * i);
					var ptr = fc.GetPointer(address, key);
					var data = new MiniEventMaster();
					string chnm = null;
					string texname = null;
					switch (i)
					{
					...
					if (ptr != 0)
					{
						...
						for (var j = 0; j < 4; j++)
						{
							...
							var ptr2 = fc.GetPointer(address2, key);
							if (ptr2 != 0)
							{
								var parts = new MiniEventParts
								{
									Part = prnm,
									Model = GetModel(fc, address2, key, $"{chnm}\\{prnm}.sa2mdl", $"{evname} {chnm} {partmetaname}")
								};
								if (parts.Model != null)
								{
				...
				var cam = fc.GetPointer(4, key);
				var camaddr = ByteConverter.ToInt32(fc, 4);
				if (cam != 0)
				{
		...
		public static void Build(bool? isBigEndian, string filename, string fileOutputPath)
		{
			...
			try
			{
				...
				var banimaddrs = new Dictionary<int, int>();
				var mdladdrs = new Dictionary<int, uint>();
				var panimaddrs = new Dictionary<int, int>();
				var pshapeaddrs = new Dictionary<int, int>();
				...
				for (var i = 0; i < 8; i++)
				{
					...
					{
						var master = evinfo.MainData[i];
						if (master.BodyAnims != null)
						{
							for (var p = 0; p < 4; p++)
							{
								var part = master.Parts[p];
								...
								if (part != null)
								{
									if (part.Anims != null)
									{
										...
										var anim = NJS_MOTION.Load(Path.Combine(Path.Combine(path, $"{master.Character}"), part.Part + ".saanim"));
									...
									if (part.ShapeMotions != null)
									{
										...
										var shape = NJS_MOTION.Load(Path.Combine(Path.Combine(path, $"{master.Character}"), part.Part + "Shape.saanim"));
										shape.OptimizeShape();
										shapebytes.AddRange(shape.GetBytes(imageBase, out var addrshape));
									...
									var partmdldata = new ModelFile(Path.Combine(Path.Combine(path, $"{master.Character}"), part.Part + ".sa2mdl")).Model;
							...
							var banimbytes = new List<byte>();
							var banim = NJS_MOTION.Load(Path.Combine(Path.Combine(path, $"{master.Character}"), "Body.saanim"));
							banimbytes.AddRange(banim.GetBytes(imageBase, out var addrbody));
	...
	public class MiniEventIniData
	{
		...
		{
			get => CharacterFlags.ToString();
			set => CharacterFlags = Enum.Parse<SA2CharacterFlags>(value);
		}
