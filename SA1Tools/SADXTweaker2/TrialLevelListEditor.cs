...
namespace SADXTweaker2
{
	public partial class TrialLevelListEditor : Form
	{
		...
		private void TrialLevelListEditor_Load(object sender, EventArgs e)
		{
			...
			foreach (KeyValuePair<string, SplitTools.FileInfo> item in Program.IniData.SelectMany(a => a.Files).Where(b => b.Value.Type.Equals("triallevellist", StringComparison.OrdinalIgnoreCase)))
			{
				string path = Path.Combine(Program.project.GameInfo.ProjectFolder, item.Value.Filename);
				levelLists.Add(new KeyValuePair<string, SA1LevelAct[]>(path, TrialLevelList.Load(path)));
