...
namespace SADXTweaker2
{
	public partial class LevelClearFlagListEditor : Form
	{
		...
		private void LevelClearFlagListEditor_Load(object sender, EventArgs e)
		{
			...
			foreach (KeyValuePair<string, SplitTools.FileInfo> item in Program.IniData.SelectMany(a => a.Files).Where(b => b.Value.Type.Equals("levelclearflags", StringComparison.OrdinalIgnoreCase)))
			{
				string path = Path.Combine(Program.project.GameInfo.ProjectFolder, item.Value.Filename);
				objectLists.Add(new KeyValuePair<string, LevelClearFlag[]>(path, LevelClearFlagList.Load(path)));
