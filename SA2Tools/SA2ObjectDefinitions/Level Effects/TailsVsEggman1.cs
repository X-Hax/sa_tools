...
{
	class TailsVsEggman1 : LevelDefinition
	{
		NJS_OBJECT model1;
		Mesh[] mesh1;
		NJS_TEXLIST skytls;
		...
		{
			model1 = ObjectHelper.LoadModel("stg20_ewvstw/models/Skybox.sa2mdl");
			mesh1 = ObjectHelper.GetMeshes(model1);
			skytls = NJS_TEXLIST.Load("stg20_ewvstw/tls/Skybox.satex");
		...
		{
			if (texs1 == null)
				texs1 = ObjectHelper.GetTextures("bgtex12", skytls, dev);
		...
		{
			if (texs2 == null)
				texs2 = ObjectHelper.GetTextures("bgtex12", watertls, dev);
			List<RenderInfo> result1 = new List<RenderInfo>();
			MatrixStack transform = new MatrixStack();
			transform.Push();
